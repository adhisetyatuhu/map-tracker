...
function RouteRow({ trackingCode, provider, addressA, addressB, status, driver }) {
    return (
            ...
            <table className="w-full bg-white shadow text-left">
                ...
                <tbody>
                    {routes.map((route, index) => (
                        <RouteRow key={index} trackingCode={route.id.split('-')[1]} provider={route.provider} addressA={route.locationA.address} addressB={route.locationB.address} status={route.status} driver={route.driver} />
                    ))}
